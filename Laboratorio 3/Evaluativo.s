...
// Recursos utilizados por el teclado
    .equ TEC_GPIO,      5
    .equ TEC_MASK,      ( S1_MASK | S2_MASK | S3_MASK )
...
contador:                   //variable para guardar estado del contador que se muestra en display
    .byte 240   //Pongo en 240 para contar las miliseg.
cuenta:
...
reset:
    ...
    LDR R6,=contador //ahi va el contador del numero
    MOV R8, 0x00 //guarda estado anterior de S1
    MOV R9, 0x00 //guarda estado anterior de S2
    MOV R10, #0x00 //guarda estado anterior de S3
...
actualizo:
    ...
    LDR R5,=TEC_MASK
    BIC R0,R5,R0        //me quedo en 1 las teclas apretadas


    // Verifica el estado del bit correspondiente a la tecla S1
    ANDS  R1,R0,#(1 << S1_BIT)
    ...
    CMP R8,#0
    ITTTT EQ        //deshabilito la cuenta
    LDREQ R5,=cuenta
    MOVEQ R11,#0
    STRBEQ R11,[R5]   //Aumento el contador
...
actualizo2:
    // Verifica el estado del bit correspondiente a la tecla S3
    ANDS  R1,R0,#(1 << S3_BIT)
    ITT NE
    MOVNE R10,#0     //actualizo estado anterior
    BNE actualizo3
    //Si antes no estaba apretada, que cuente
    CMP R10,#0
    ITTTT EQ            //resetea la cuenta a 24.0
    LDREQ R5,=contador
    MOVEQ R11,#240
    STRBEQ R11,[R5]   //Aumento el contador
    MOVEQ R10,#1   //actualizo estado anterior

actualizo3:

    // Repite el lazo de actualizacion indefinidamente
    B actualizo
...
systick_isr:
    ...
    PUSH {R4-R11}
    LDR R10, =segundo       //segundo lleva la cuenta del timer
    LDRB R11, [R10]
    ...
    LDR R3,=contador
    LDRB R2,[R3]            //obtengo valor de la cuenta

    CMP R2, #0          //chequea si la cuenta termino
    BEQ termino

    //chequea si la cuenta esta deshabilitada
    LDR R4, =cuenta
    LDRB R4,[R4]
    CMP R4, #0
    BEQ refresh
    ...
    ITTE EQ
    SUBEQ R2,#1
    MOVEQ R11, #16        //aca va la cantidad de ciclos
    SUBNE R11, #1
    B sigo

termino:                         //prende led cuando cuenta es 0
    LDR R5,=GPIO_PIN0
    MOV R4,#0
    ORR R4,#(1 << LED_G_BIT)
...
sigo:                         //prende led cuando cuenta no es 0
    LDR R5,=GPIO_PIN0
    MOV R4,#0
    STR   R4,[R5,#(LED_GPIO << 2)]
    B refresh
...
decenas:
    CMP R1,#2
    BNE segundos
    LDR R6, =DIG_2_MASK     //Apago
    STR R6,[R4,#(DIG_GPIO << 2)]
    LDR R6, =DIG_3_MASK     //Apago
    STR R6,[R4,#(DIG_GPIO << 2)]
    MOV R4,#10
    UDIV R9, R2, R4 //Obtengo segundos
    MUL R9,R4
    SUB R5, R2,R9
    LDR R9, [R7,R5]                 //obtengo el digito
    STRB R9,[R8,#(SEG_GPIO << 2)]   //Cargo el valor

    LDR R5,=GPIO_SET0
    LDR R6, =DIG_1_MASK     //Habilito
...
segundos:
    ...
    ITT HI      
    SUBHI R5, #20
    BHI muestro   // p obtener segundos 
    CMP R5, #9
    IT HI
    SUBHI R5, #10
    BHI muestro   // p obtener segundos 
...
muestro:
    ...
    LDR R8,=GPIO_PIN0
    STRB R9,[R8,#(SEG_GPIO << 2)]
    LDR R6, =DIG_2_MASK   //habilito segundos
    STR R6,[R5,#(DIG_GPIO << 2)]
...
miliseg:
    CMP R1,#0
    BNE systick_exit
    LDR R6, =DIG_2_MASK     //Apago decenas
    STR R6,[R4,#(DIG_GPIO << 2)]
    LDR R6, =DIG_3_MASK     //Apago segundos
    STR R6,[R4,#(DIG_GPIO << 2)]
    MOV R4,#100
    UDIV R5, R2, R4              //obtengo la decena
    LDR R9, [R7,R5]                 
    STRB R9,[R8,#(SEG_GPIO << 2)]   //Cargo
    LDR R5,=GPIO_SET0
    LDR R8,=GPIO_PIN0
    STRB R9,[R8,#(SEG_GPIO << 2)] //Cargo 
    LDR R6, =DIG_3_MASK
