...
reset:
    ...
    // Configura los pines de los leds como gpio sin pull-upw
    LDR R1,=SCU_BASE
...
refrescar:
    ...
    LDRB R7, [R6] //obtengo valor contador
    ...
    CMP R8,#0
    ITT EQ
    ADDEQ R7,#1   //Aumento el contador
...
refrescar1:
    ...
    CMP R9,#0
    ITT EQ
    SUBEQ R7,#1   //Aumento el contador
...
systick_isr:
    ...
    */
    ...
    LDR R2,=contador
    LDRB R2,[R2]            //obtengo valor de la cuenta
    LDR R4,=GPIO_CLR0
    LDR R5,=GPIO_SET0
    LDR R8,=GPIO_PIN0
    ADR R7, tabla
    LDR R7, [R7,R2,LSL #2]  //obtengo los cuatro digitos a mostrar

dig1:
    CMP R1,#4
    BNE dig2

    //deshabilito digito cuatro
    LDR R6, =DIG_4_MASK
...
dig2:
    ...
    //obtengo valor de segmentos para digito dos
    MOV R9, R7
    LSR R9,R9,#8
    AND R9,0xFF
...
dig3:
    ...
    //deshabilito digito dos
    LDR R6, =DIG_2_MASK
    STR R6,[R4,#(DIG_GPIO << 2)]

    //obtengo valor de segmentos para digito tres
    MOV R9, R7
    LSR R9,R9,#16
    AND R9,0xFF
...
dig4:
    ...
    //deshabilito digito tres
    LDR R6, =DIG_3_MASK
    STR R6,[R4,#(DIG_GPIO << 2)]

    //obtengo valor de segmentos para digito cuatro
    MOV R9, R7
    LSR R9,R9,#24

    //cargo valor de segmentos para digito cuatro
    STRB R9,[R8,#(SEG_GPIO << 2)]
    //habilito digito cuatro
    LDR R6, =DIG_4_MASK
    STR R6,[R5,#(DIG_GPIO << 2)]
...
reiniciaCont:
    CMP R1,#0
    BNE systick_exit
    MOV  R1,#5             // Vuelvo a carga espera con 5 iteraciones
*/
...
.pool
tabla:  //los numeros van de izq a der
        .byte 0x3F,0x3F,0x3F,0x3F //0
        .byte 0x06,0x3F,0x3F,0x3F //1
        .byte 0x3F,0x06,0x3F,0x3F //2
        .byte 0x06,0x06,0x3F,0x3F //3
        .byte 0x3F,0x3F,0x06,0x3F //4
        .byte 0x06,0x3F,0x06,0x3F //5
        .byte 0x3F,0x06,0x06,0x3F //6
        .byte 0x06,0x06,0x06,0x3F //7
        .byte 0x3F,0x3F,0x3F,0x06 //8
        .byte 0x06,0x3F,0x3F,0x06 //8
