...
// Recursos utilizados por los digitos
    .equ DIG_GPIO,      0
    .equ DIG_MASK,      ( DIG_1_MASK | DIG_2_MASK )
/****************************************************************************/
...
reset:
    ...
    STR R0,[R1,#(LED3_PORT << 7 | LED3_PIN << 2)]
...
/*
    ...
    //que el led3 empiece prendido y el 4 apagado
    MOV R0,#2
    //toggle on match del led 4
    ORR R0,#(3 << (4 + (2 * LED4_MAT)))
    //toggle on match del led 3
    ORR R0,#(3 << (4 + (2 * LED3_MAT)))
    STR R0,[R1,#EMR]
*/

    //Defino estado inicial todo 0 y
    //toggle on match del led 4
...
digito1:
    //cambio de digito para la prox
    MOV R1,#1
    STRB R1,[R0]
/*
    LDR R0, =estado
    LDRB R1, [R0]
    //hago el cambio de estado y salto al que corresponde
    CMP R1, #0
    ITTT EQ
    MOVEQ R2,#1
    SUBEQ R2,R1
